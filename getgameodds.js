...
function buildGameSummaryLibrary(gameSummaryData) {
    //console.log('got here', JSON.parse(gameSummaryData))
    //jsonData = JSON.parse(gameSummaryData)
    console.log(typeof(gameSummaryData))
    ...
    const gameSummaryObject = {
        ...
        gameSummaryTime: parsedJsonData.gameData.datetime.dateTime,
        ...
        gameSummaryLastUpdated: new Date(),
    }
    console.log('gameSummaryObject: ', gameSummaryObject)
    return gameSummaryObject
}

function writeGameSummaryToFile(gameLibrary) {
    //console.log(gameLibrary)
    //console.log(libraryPath)
    fs.writeFileSync(libraryFile, JSON.stringify(gameLibrary), (err) => {
        if (err) {
            console.log(err)
        }      
    })
...
function CheckLibraryStatus() {
    try {
        if (fs.existsSync(libraryFile)) {
            ...
            console.log("No Library found.  This won't work")
        }
    } catch(err) {
        console.log(err)
    }
...
async function BuildGameSummaryLibraryFromScratch() {
    ...
    writeGameSummaryToFile(gameSummaryLibrary)
}
/////////////////////////////////////////////////////////////
...
function buildTodaysScheduleURL() {
    ...
    todaysScheduleDatePST = new Date(todaysScheduleDate.getTime() - (todaysScheduleDate.getTimezoneOffset()*60*1000))  //PST is the base for North America since EST is BS
    console.log('Time PST: ', todaysScheduleDatePST)
    todaysScheduleDate = todaysScheduleDatePST.toISOString().substring(0,10);
